...
describe('AboutMeComponent', () => { // 測試環境
  ...
  beforeEach(async () => { // 每個it 執行前會觸發
    await TestBed.configureTestingModule({ // 每個測試的獨立module環境
...
describe('測試案例群組', () => {
	...
	it('測試案例內容 2', () => { expect(true).toBe(true);
	});
...
[Matchers清單](https://jasmine.github.io/api/edge/matchers.html)
